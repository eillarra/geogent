...
from geojson import FeatureCollection, Feature, Point, Polygon  # type: ignore
...
def parse_coordinates(coordinates: str) -> Union[Point, Polygon]:
    points = []
...
def page_not_found(e) -> Response:
    res = json.dumps({"error": "Sorry, but we could not find the requested page"})
...
def get_dataset(category: str, dataset: str) -> Response:
    key = f"{category}/{dataset}"

    try:
        source = get(f"{DATATANK}{key}.kml").text.encode("utf-8")
    ...
    for el in tree.xpath("//kml:Placemark", namespaces=nsm):
        feature_id = el.attrib["id"]
        properties = {}

        for prop in el.xpath(".//kml:SimpleData", namespaces=nsm):
            value = prop.text
            try:
                properties[prop.attrib["name"].lower()] = (
        ...
        features.append(Feature(id=feature_id, geometry=geometry, properties=properties))
    ...
    res = json.dumps(
        FeatureCollection(
            ...
            properties={
                "title": tree.xpath(
                    "//kml:Document/kml:Folder/kml:name", namespaces=nsm
                )[0].text,
                "source": f"{DATATANK}{key}",
            },
        )
    )
